...
define('IN_AC',true);
//定义个常量，用来指定本页的内容
define('SCRIPT','adm_current_project_pass');
//引入公共文件
...
_page("SELECT pro_id FROM project WHERE pro_state='通过' AND pro_year='$_year'",10);   //第一个参数获取总条数，第二个参数，指定每页多少条
$_result = _query("SELECT
											pro_id,pro_name,pro_applicant,pro_stuNumber,pro_college,pro_sort,pro_teacher,pro_adjudicator,pro_url
							  FROM
											project
							WHERE
											pro_state='通过' AND pro_year='$_year'
						ORDER BY
											pro_No
							   LIMIT
											$_pagenum,$_pagesize
...
<div id="body">
	...
	<div id=main>
		<h2>当前项目列表</h2>
			...
			<div id=table>
			<table cellspacing="1">
				<tr><th width=39%>项目名称</th><th width=10%>申报人</th><th width=11%>学院</th><th width=10%>项目分类</th><th>指导教师</th><th>评审教师</th><th>项目文档</th></tr>
					<?php 
					...
					if($_html['pro_adjudicator']==''){echo '待分配';} else echo $_html['pro_adjudicator']?></td><td><a href="adm_stu_doc.php?stuNum=<?php echo $_html['pro_stuNumber']?>">查看</a></td></tr>
