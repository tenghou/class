...
if ($_GET['action'] == 'distribute' && isset($_POST['ids'])) {
	...
	if (!!$_rows = _fetch_array("SELECT 
				...
				")){
						...
						if (_affected_rows()) {
						_close();
						_location('分配成功','adm_search_current.php');
						} else {
							_close();
							_alert_back('分配失败');
						}
...
_page("SELECT pro_id FROM project WHERE pro_year='$_year' AND ".$_pro." LIKE '%{$_clean['keyword']}%'",9);   //第一个参数获取总条数，第二个参数，指定每页多少条
			...
			<div id=search>
				<form method="get" name="adm_search" action="adm_search_current.php">
					<select class="select" name="type">
						...
						<option <?php if($type==proteacher) echo 'selected="selected"' ?> value="proteacher">按指导教师</option>
						<option <?php if($type==proadjudicator) echo 'selected="selected"' ?> value="proadjudicator">按评审教师</option>
					</select>
