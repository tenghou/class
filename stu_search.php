...
_page("SELECT pro_id FROM project WHERE pro_stuNumber = '{$_COOKIE['stu_number']}' AND pro_year <'$_year' AND ".$_pro." LIKE '%{$_clean['keyword']}%'",10);   //第一个参数获取总条数，第二个参数，指定每页多少条
$_result = _query("SELECT
										pro_name,pro_applicant,pro_college,pro_sort,pro_year,pro_teacher,pro_state,pro_url
							 FROM
										project
						   WHERE
										pro_stuNumber = '{$_COOKIE['stu_number']}' AND pro_year <'$_year' AND ".$_pro." LIKE '%{$_clean['keyword']}%'
					   ORDER BY
										pro_year DESC
							  LIMIT
										$_pagenum,$_pagesize
		...
		<div id=search>
				<form method="get" name="stu_search" action="stu_search.php">
					<select class="select" name="type">
						...
						<option <?php if($type==proteacher) echo 'selected="selected"' ?> value="proteacher">按指导老师</option>
					</select>
			...
			<div id=table>
				<table cellspacing="1">
					<tr><th width=40%>项目名称</th><th>申报人</th><th>学院</th><th>项目分类</th><th>年度</th><th>指导老师</th><th>申报状态</th><th>操作</th></tr>
					<?php 
