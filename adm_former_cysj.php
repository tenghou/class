...
_page("SELECT pro_id FROM project WHERE pro_year !='$_year' AND pro_sort ='创业实践项目'",10);   //第一个参数获取总条数，第二个参数，指定每页多少条
$_result = _query("SELECT
											pro_id,pro_name,pro_applicant,pro_stuNumber,pro_college,pro_sort,pro_year,pro_teacher,pro_state,pro_url
							  FROM
											project
							WHERE
											pro_year !='$_year' AND pro_sort ='创业实践项目'
						ORDER BY
											pro_year DESC
							   LIMIT
											$_pagenum,$_pagesize
...
<title>四川大学科技项目管理平台-往期创业实践项目列表</title>
<?php 
